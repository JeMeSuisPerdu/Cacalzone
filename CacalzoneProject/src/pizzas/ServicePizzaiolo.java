...
/**
 * Service gérant la logique du Pizzaiolo en utilisant le Menu.
 */
public class ServicePizzaiolo implements InterPizzaiolo {
    ...
    public List<Commande> commandeNonTraitees() {
        ...
        // Marquer comme traitées lors de la lecture selon le sujet
        aTraiter.forEach(Commande::traiter);
        return aTraiter;
    }

    @Override public int creerIngredient(String n, double p) { 
        if (n == null || n.isBlank()) return -1;
        if (p <= 0) return -3;
        menu.getIngredients().add(new Ingredient(n, p));
        return 0;
    }
    
    @Override public Set<Pizza> getPizzas() { 
    ...
    public int ajouterIngredientPizza(Pizza pizza, String nomIngredient) {
        ...
        if (ingredientAtrouver.getTypesPizzaInterdits().contains(pizza.getTypePizza())) {
            return -3;
        }
    ...
    public List<Commande> commandesTraiteesClient(InformationPersonnelle client) {
        ...
        for (Commande c : menu.getCommandesGlobales()) {
            if (c.getEtat() == Commande.Etat.TRAITEE && c.getClient().getInfos().equals(client)) {
                resultat.add(c);
            }
        }
        return resultat;
    ...
    public List<Pizza> classementPizzasParNombreCommandes() {
        ...
        classement.sort((p1, p2) -> ventesParPizza.get(p2).compareTo(ventesParPizza.get(p1)));
